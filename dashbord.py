...
Export Excel des collectes du jour (corrigé).
"""
...
def exporter_collectes_agent(date_filter, agent_nom):
    ...
    try:
        with engine.connect() as conn:
            ...
            if df.empty:
                return None
            # Convertir les colonnes time en chaînes HH:MM
            for col in ['heure_depot_depart', 'heure_retour_depot']:
                if col in df.columns:
                    df[col] = df[col].apply(lambda x: x.strftime("%H:%M") if pd.notna(x) else "")
            # Calcul de durée sécurisé
            def calc_duree(row):
                try:
                    depart = row['heure_depot_depart']
                    retour = row['heure_retour_depot']
                    if depart and retour and depart != '' and retour != '':
                        def to_minutes(t):
                            parts = t.split(':')
                            h = int(parts[0])
                            m = int(parts[1]) if len(parts) > 1 else 0
                            return h*60 + m
                        depart_min = to_minutes(depart)
                        retour_min = to_minutes(retour)
                        minutes = retour_min - depart_min
                        if minutes < 0:
                            minutes += 24*60
    ...
    if st.button("📥 EXPORTER MES COLLECTES DU JOUR", use_container_width=True):
        if st.session_state.agent_nom:
            ...
            if df is not None and not df.empty:
                ...
                if df is None:
                    st.error("Erreur lors de l'export. Vérifiez les données.")
                else:
