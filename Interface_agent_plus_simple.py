...
Version améliorée pour une meilleure accessibilité
- Icônes pour faciliter la lecture
- Boutons plus gros et colorés
- Messages clairs
...
with st.sidebar:
    ...
    if st.session_state.gps_actif:
        st.markdown('<div class="gps-active">📍 GPS ACTIF - Votre position sera enregistrée</div>', unsafe_allow_html=True)
...
if not st.session_state.collecte1_validee:
    ...
    with col2:
        if st.button("📍 ENREGISTRER LE DÉPART", key="btn_depart_decharge1"):
            pos = get_position()
    ...
    with col2:
        if st.button("📍 ENREGISTRER L'ARRIVÉE", key="btn_arrivee_decharge1"):
            pos = get_position()
    ...
    with col2:
        if st.button("💾 ENREGISTRER VOLUME", key="btn_sortie1"):
            pos = get_position()
            if volume1_input > 0:
...
if st.session_state.collecte1_validee and not st.session_state.collecte2_validee:
    ...
    with col2:
        if st.button("📍 ENREGISTRER LE DÉPART", key="btn_depart_decharge2"):
            pos = get_position()
    ...
    with col2:
        if st.button("📍 ENREGISTRER L'ARRIVÉE", key="btn_arrivee_decharge2"):
            pos = get_position()
    ...
    with col2:
        if st.button("💾 ENREGISTRER VOLUME", key="btn_sortie2"):
            pos = get_position()
            if volume2_input > 0:
...
if st.session_state.points_gps:
    ...
    🟢 <strong>Vert</strong> - Départ dépôt<br>
    🔵 <strong>Bleu</strong> - Points de collecte (début/fin)<br>
    🟠 <strong>Orange</strong> - Départ vers décharge<br>
    🔴 <strong>Rouge</strong> - Arrivée décharge<br>
    🟣 <strong>Violet</strong> - Sortie décharge<br>
    🟤 <strong>Marron</strong> - Retour dépôt<br>
    🔵 <strong>Ligne bleue</strong> - Trajet effectué
    </div>
    """, unsafe_allow_html=True)
